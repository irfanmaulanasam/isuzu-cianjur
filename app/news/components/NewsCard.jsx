...
export default function NewsCard({ article }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 border border-gray-200 dark:border-slate-700 flex flex-col h-full">
      <div className="p-6 flex flex-col flex-grow">
        {/* Category Badge */}
        <span
          className={`inline-block px-3 py-1 rounded-full text-xs font-medium mb-3 ${categoryColor} self-start`}
        >
          {article.category || "article"}
        </span>

        {/* Title */}
        <h3 className="text-xl font-semibold text-gray-800 dark:text-slate-100 mb-3 line-clamp-2">
          <Link
        ...
        {/* Excerpt */}
        {article.excerpt && (
          <p className="text-gray-600 dark:text-slate-300 line-clamp-3 mb-4 flex-grow">
            {article.excerpt}
          </p>
        )}
        ...
        <div className="mt-auto">
          <Link
            href={`${categoryPath}/${article.slug}`}
            className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-semibold text-sm inline-flex items-center"
          >
            Baca Selengkapnya →
          </Link>
        </div>
